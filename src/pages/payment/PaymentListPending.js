...
const propertyIDHandleChange = ( e ) => {
  ...
  const columns = [
	...
	
	{
		name: "Actions",
		selector: row => {
			return <div> <Link to={"/payment/status/recorded/" + row.id} className="theme-btn-edit red"> RP</Link> ||  <Link to={"/payment/" + row.id} className="theme-btn-edit">Edit</Link> </div>
		}
	  }
];
    ...
    useEffect(() => {
      ...
      const fetchPayments = () => {
        const api = 'https://api.americanbestit.com/api/v1/payments/list/pending'; 
        const token = localStorage.getItem('access_token');

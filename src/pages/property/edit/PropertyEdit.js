...
const PropertyEdit = () => {
	...
	const fetchProperty = async () => {
		const api = 'http://localhost:3000/api/v1/properties/'+id; 
	...
	const updateProperty = () => {
		...
		const api = 'http://localhost:3000/api/v1/property/update/' + id;
		const token = localStorage.getItem('access_token');
		axios({
			method: 'post',
			url: api,
	...
	const updatePropertyImage = () => {
        ...
        const api = 'http://localhost:3000/api/v1/propertyimages';
        const token = localStorage.getItem('access_token');
